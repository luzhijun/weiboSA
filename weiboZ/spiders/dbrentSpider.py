...
class dbrentSpider(CrawlSpider):
    ...
    def __init__(self, num=100, new_url='', *args, **kwargs):
        ...
        self.start_urls=[self.url_temp+str(i) for i in range(2,min(int(num),self.maxPage))]
    ...
    def parse_item(self,response):
        ...
        context=sel.css('#content > div > div.article > div.topic-content.clearfix > div.topic-doc')
        # 发布人
        DI['user']=context.css("h3 > span.from > a::text").extract()[0]
        #时间
        DI['created_at']=context.css(' h3 > span.color-green::text').extract()[0]
        # 标题，可能两种匹配，优先选择正文中的
        title=context.css('table > tbody > tr:nth-child(2) > td.tablecc').extract()
        if not title:
